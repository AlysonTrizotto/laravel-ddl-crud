...
class MakeCrudFromDdl extends Command
{
     ...
     */
    protected $signature = 'make:crud-from-ddl';
    ...
    {
        // Prompt domain (required) and enforce CamelCase (StudlyCase)
        $domainInput = $this->ask('Informe o domínio (CamelCase)');
        ...
        // Ask for DDL file path
        $ddlPath = $this->ask('Caminho do arquivo DDL (.sql)');
    ...
    protected function columnToBlueprint(array $col): string
    {
        ...
        if ($default) {
            ...
            } elseif (preg_match("/^'(.*)'$/", $default, $sm)) {
                $val = str_replace("'", "\\'", $sm[1]);
                $code .= "->default('{$val}')";
            } else {
...
PHP;
        $content = str_replace(['{$ns}', '{$class}', '{$rulesExport}'], [$ns, $class, $rulesExport], $template);
        File::put($path, $content);
